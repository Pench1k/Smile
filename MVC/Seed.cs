...
namespace MVC
{
    public class Seed
    {
        ...
        public void SeedDataContext()
        {
            ...
            //        }
            //    };
            //    _context.Stomatology.AddRange(stomatology);
            ...
            //            }
            //    };
            //    _context.Office.AddRange(office);
            //    _context.SaveChanges();
            //}
            ...
            //    };
            //    _context.Specialization.AddRange(specialization);
            //    _context.SaveChanges();
            //}
            ...
            if (!_context.Doctor.Any())
            {
                var doctors = new List<Doctor>
                {
                    new Doctor
                    {
                        ...
                        WorkersId = _context.Workers.FirstOrDefault(w => w.UserId == _context.Users.FirstOrDefault(u => u.UserName == "alexey.morozov").Id)?.Id
                    },
                    new Doctor
                    {
                        ...
                        WorkersId = _context.Workers.FirstOrDefault(w => w.UserId == _context.Users.FirstOrDefault(u => u.UserName == "elena.kuzmina").Id)?.Id
                    },
                    new Doctor
                    {
                        SpecializationId = _context.Specialization.FirstOrDefault(s => s.Name == "Терапевт")?.Id,
                        OfficeId = _context.Office.FirstOrDefault(o => o.Name == "Кабинет 4")?.Id,
                        WorkersId = _context.Workers.FirstOrDefault(w => w.UserId == _context.Users.FirstOrDefault(u => u.UserName == "pavel.mikhaylov").Id)?.Id
                    },
                    new Doctor
                    {
                        SpecializationId = _context.Specialization.FirstOrDefault(s => s.Name == "Ортопед")?.Id,
                        OfficeId = _context.Office.FirstOrDefault(o => o.Name == "Кабинет 3")?.Id,
                        WorkersId = _context.Workers.FirstOrDefault(w => w.UserId == _context.Users.FirstOrDefault(u => u.UserName == "daria.karpova").Id)?.Id
                    },
                    new Doctor
                    {
                        SpecializationId = _context.Specialization.FirstOrDefault(s => s.Name == "Терапевт")?.Id,
                        OfficeId = _context.Office.FirstOrDefault(o => o.Name == "Кабинет 2")?.Id,
                        WorkersId = _context.Workers.FirstOrDefault(w => w.UserId == _context.Users.FirstOrDefault(u => u.UserName == "vladimir.korolev").Id)?.Id
                    },
                    new Doctor
                    {
                        SpecializationId = _context.Specialization.FirstOrDefault(s => s.Name == "Ортопед")?.Id,
                        OfficeId = _context.Office.FirstOrDefault(o => o.Name == "Кабинет 1")?.Id,
                        WorkersId = _context.Workers.FirstOrDefault(w => w.UserId == _context.Users.FirstOrDefault(u => u.UserName == "irina.fedorova").Id)?.Id
                    }
                };

                // Проверка на null для каждого нового доктора перед добавлением в базу данных
                foreach (var doctor in doctors)
                {
                    if (doctor.SpecializationId == null || doctor.OfficeId == null || doctor.WorkersId == null)
                    {
                        // Пропускаем доктора, если есть ошибка
                        continue;
                    }
                }
